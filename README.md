...
- **iCloud 同步完全移除**：資料只存在這台裝置。舊版若曾啟用過 iCloud，第一次開 v2.0 會自動把紀錄搬回本機並封存舊檔。
- **語音記帳**：講一句話 → 裝置端辨識 → 規則解析（中文數字、台灣用語詞庫、拼音模糊比對）→ 自動打開確認畫面填好等你確認。
- **電子發票 QR 解析**：收據辨識會先掃 QR，金額與日期直接取自發票。
- **備份與還原**：取代 iCloud 的角色，換手機靠這個。
- **本地通知**、**Face ID 鎖**、**互動式 Widget**、**分類詳情頁**。

## （v1）iCloud 同步為什麼會「開了就閃退」
