...
(function () {
  ...
  (function () {
    ...
    audio.addEventListener('ended', function () { btn.classList.remove('is-on'); });
    ...
    btn.addEventListener('click', function () {
      ...
      function fail() {
        ...
        clearTimeout(timer);
        btn.classList.remove('is-on');
        toast('音檔還沒放上去（assets/chaichai.mp3）');
      }

      var timer = setTimeout(function () { if (audio.paused) fail(); }, 1500);
      audio.addEventListener('playing', ok, { once: true });
      audio.addEventListener('error', fail, true);
