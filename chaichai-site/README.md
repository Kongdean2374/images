...
> 🎵 **音樂播放器**
>
> 左下角（手機是最下面一條）常駐一個小卡片，上面寫「配著自介一起食用」。
> 封面是刻意做成黑白、看不出是什麼歌的，**整塊都可以點**，
> 點下去才會載入 YouTube 播放器，就在原地播放，不會跳走。
>
> - 沒人點的話，YouTube 的程式**完全不會載入**，不拖慢網站速度
> - 按右上角 ✕ 或 Esc 就會關掉，關掉時播放器會整個移除，音樂是真的停掉
> - 語錄區那顆 🐾 也接到同一個播放器
>
> **要換歌**：打開 `script.js`，最上面找到 `var YT_ID = 'dQw4w9WgXcQ';`，
> 把引號裡那串換成你要的影片 ID —— 就是 YouTube 網址 `watch?v=` 後面那一段。
>
> **要改播放器上的字**：在 `index.html` 搜尋 `player__label`（「配著自介一起食用」）
> 和 `player__hint`（「點這塊任何地方就會開始播」）。
>
> 用嵌入的方式就沒有版權問題——播放次數算在原作者頭上，
> 跟你自己放一個 mp3 上去是完全不同的兩件事。
