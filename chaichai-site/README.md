...
> 🎵 **音樂播放器**
>
> 左下角常駐一張小卡片（收起來只有 62px 高，手機約佔畫面 7%），
> 寫著「配著自介一起食用」，封面刻意做成黑白、看不出是什麼歌。
>
> **點一下就會開始播**，不用點兩次。做法是把 YouTube 播放器藏在封面底下，
> 封面本身不吃點擊，所以手指其實是直接點在 YouTube 的播放鍵上——
> 對瀏覽器來說這是「iframe 內的真實操作」，iPhone 才不會擋掉播放。
> 音樂一開始播，封面才淡掉、展開成完整播放器（約佔畫面 19%），
> 右上角會出現 ✕，按它或 Esc 就關掉，關掉時 iframe 會被移除，音樂是真的停。
>
> **要換歌**：打開 `script.js`，找到 `var YT_ID = 'dQw4w9WgXcQ';`，
> 把引號裡那串換成你要的影片 ID —— 就是 YouTube 網址 `watch?v=` 後面那一段。
>
> **要改播放器上的字**：在 `index.html` 搜尋 `player__label`（「配著自介一起食用」）
> 和 `player__hint`（「點這塊任何地方就會開始播」）。
>
> 小提醒：為了做到「只點一次」，YouTube 播放器必須在開頁時就先準備好，
> 所以每次開頁會多一個對 YouTube 的請求（只有封面圖，影片本身沒點不會載）。
> 用的是 `youtube-nocookie.com`，訪客沒點就不會被種 cookie。
