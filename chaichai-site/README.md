...
> 封面刻意做成黑白、看不出是什麼歌。
>
> **點一下就會開始播。** 做法是 YouTube 播放器完整鋪在下面，
> 上面那層封面設成不吃點擊，所以手指其實是直接點在 YouTube 的播放鍵上——
> 對瀏覽器來說這是「iframe 內的真實操作」，iPhone 才不會擋。
> 音樂開始播，封面才淡掉、露出播放器，右上角出現 ✕。按 ✕ 或 Esc 關掉，
> 關掉時 iframe 會被移除，音樂是真的停。
>
...
>
> 小提醒：為了做到只點一次，播放器必須在開頁時就先備好，
> 所以每次開頁會多一個對 YouTube 的請求（只有封面，影片沒點不會載）。
> 用 `youtube-nocookie.com`，沒點就不會被種 cookie。
