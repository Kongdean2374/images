...
> 🔊 **音檔還沒放，所以畫面上看不到播放器。**
>
> 把你要的音檔改名成 **`chaichai.mp3`**，丟進 `assets/` 資料夾就好，不用改任何程式碼。
> 網頁一載入會自己去找這個檔案：
>
> - **找得到** → 左下角（手機是最下面一條）出現常駐播放器，寫著「配著自介一起食用」，
>   可以點播放、拖進度、調音量，音量設定會記住。語錄區的 🐾 也會接到同一個播放器。
> - **找不到** → 播放器和 🐾 按鈕會自動整個隱藏，訪客不會看到一個按不動的東西。
>
> 想讓音樂重複播放的話，打開 `index.html` 搜尋 `id="pl-audio"`，
> 把那行改成 `<audio id="pl-audio" preload="none" loop>` 即可。
>
> 播放器上那句「配著自介一起食用」要改字的話，搜尋 `player__label`。
